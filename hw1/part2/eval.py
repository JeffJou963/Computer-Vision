...
def main():
    ...
    print('[Error] Bilateral: %d\n[Error] Joint bilateral: %d'% (bf_error, jbf_error))

    # bf_out = cv2.cvtColor(bf_out, cv2.COLOR_BGR2RGB)
    # jbf_out = cv2.cvtColor(jbf_out, cv2.COLOR_BGR2RGB)
    # cv2.imwrite('./save/bf_.png',bf_out)
    # cv2.imwrite('./save/jbf_.png',jbf_out)

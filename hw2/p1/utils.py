...
def build_vocabulary(img_paths, vocab_size): #400
    ...
    for img_path in tqdm(img_paths):
        ...
        # feat.shape (3763/4294, 128)
        _, feat = dsift(img, step=[2,2], fast=True)
        feats.append(feat)
    # feats: 
    #  feat x 1500
    #  shape=(5845608, 128)
...
def get_bags_of_sifts(img_paths, vocab):
    ...
    for img_path in tqdm(img_paths):
        img = np.array(Image.open(img_path))
        _, sift_feat = dsift(img, step=[2,2], fast=True)
        # (xxx, vocab_size)
        distances = cdist(sift_feat, vocab)
        nearest_centers = np.argmin(distances, axis=1)
        histogram, _ = np.histogram(nearest_centers, bins=len(vocab))
...
def nearest_neighbor_classify(train_img_feats, train_labels, test_img_feats):
    ...
    for distance_vector in distances:
        nearest_indices = np.argsort(distance_vector)[:k]  # Get indices of k smallest distances
        nearest_labels = [train_labels[index] for index in nearest_indices]  # Extract the labels of these nearest neighbors
        ...
        # Sort labels by their count and return the label with the highest count
        # In case of ties, this method returns the first encountered maximum count label

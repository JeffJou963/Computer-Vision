    ...
    for epoch in range(cfg.epochs):
        ...
        with torch.no_grad():
            ...
            for batch, data in enumerate(val_loader):
                ...
                sys.stdout.flush()
                # Data loading.
                val_images, val_labels = data['images'].to(device), data['labels'].to(device) # (batch_size, 3, 32, 32), (batch_size)
                # Forward pass. input: (batch_size, 3, 32, 32), output: (batch_size, 10)
                pred = model(val_images)
                # Calculate loss.
                val_loss = criterion(pred, val_labels)
                # Evaluate.
                val_correct += torch.sum(torch.argmax(pred, dim=1) == val_labels)
